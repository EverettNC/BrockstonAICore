"""Multi-path reasoning dispatcher coordinating BROCKSTON's cortex tools."""
...
def _load_cortex_bundle() -> Optional[Dict[str, Any]]:
    try:
        ...
        return None
    return {
        "BROCKSTONCortex": getattr(module, "BROCKSTONCortex", None),
...
def _load_advanced_local() -> Optional[type]:
    try:
        module = import_module("local_reasoning_engine")
    except ImportError:  # pragma: no cover - defensive
        return None
...
class ReasoningDispatcher:
    ...
    def __init__(
        ...
        if (
            cortex_bundle
            and cortex_bundle.get("BROCKSTONCortex")
            and cortex_bundle.get("ReasonerConfig")
        ):
            try:
                self.cortex = cortex_bundle["BROCKSTONCortex"](
