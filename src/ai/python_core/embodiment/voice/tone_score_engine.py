...
class ToneScoreEngine:
    ...
    def analyze_tone(self, audio_path: str) -> Dict:
        ...
        if not _librosa_ok:
            logger.warning("librosa not installed — providing cognitive estimate fallback")
            return {
                "arousal": 50, "valence": 50, "dominance": 50,
                "emotions": {lbl: 0.14 for lbl in self.emotion_labels},
                "emotion_intensity": 0, "tone_score": 50,
                "interpretation": "balanced (cognitive estimate)",
                "response_mode": self.adaptive_response_mode(50),
                "physiological": {"pitch_mean": 0, "jitter": 0, "shimmer": 0, "hnr": 0},
                "warning": "librosa missing — results are estimated"
            }
