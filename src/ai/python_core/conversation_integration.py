...
class ConversationIntegration:
    ...
    def _load_components(self):
        ...
        try:
            ...
            self.components["conversation_handler"] = get_conversation_handler(
                self.user_id
            )
    ...
    def start(self) -> bool:
        ...
        if conversation_handler:
            ...
            # Register callbacks
            conversation_handler.register_output_callback(
                self._handle_conversation_output
            )
    ...
    def _handle_speech_input(
        self, text: str, confidence: float, metadata: Dict[str, Any]
    ):
        """
    ...
    def process_text_input(
        ...
        if not self.is_active:
            logger.warning(
                "Cannot process input: Conversation integration is not active"
            )
    ...
    def process_audio_data(
        ...
        if not self.is_active:
            logger.warning(
                "Cannot process audio: Conversation integration is not active"
            )
    ...
    def process_eye_tracking(
        ...
        if not self.is_active:
            logger.warning(
                "Cannot process eye tracking: Conversation integration is not active"
            )
            return {"error": "Conversation system is not active"}
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        status = {
            ...
            "components": {
                ...
                "conversation_engine": (
                    "loaded"
                    if "conversation_engine" in self.components
                    else "not_loaded"
                ),
                "speech_recognition": "loaded" if speech_recognition else "not_loaded",
                "conversation_handler": (
                    "loaded" if conversation_handler else "not_loaded"
                ),
                "lstm_interpreter": (
