...
class KnowledgeEngine:
    ...
    def reason(self, query: str, context: Optional[str] = None) -> Dict[str, Any]:
        """Provides reasoning over the knowledge graph."""
        logger.info(f"Knowledge Engine reasoning over: {query}")
        
        # Search facts
        results = []
        for fact in self.fact_manager.facts:
            if query.lower() in fact['text'].lower() or any(t.lower() in query.lower() for t in fact['topics']):
                results.append(fact)
