...
import { Radio, Zap, Heart, Waves, Settings, Wifi, WifiOff } from 'lucide-react';
    ...
    <Card className="bg-card/50 backdrop-blur-sm border-white/5">
      <CardHeader className="pb-2">
        <CardTitle className="flex items-center justify-between text-sm font-headline uppercase tracking-wider text-secondary">
          ...
          {connected ? (
            <Badge variant="outline" className="text-[8px] border-accent/40 text-accent flex items-center gap-1">
      ...
      <CardContent className="space-y-4">
        {!connected && (
          <Button 
            ...
            variant="outline" 
            size="sm" 
            className="w-full text-[10px] border-accent/20 text-accent hover:bg-accent/10 h-8"
          >
        ...
        <div className="text-[9px] font-code text-secondary/40 border-t border-white/5 pt-2">
          <span>Carbon-Silicon Symbiosis: Tactile Bridge Active</span>
        </div>
      </CardContent>
    </Card>
  );
    ...
    <button
      ...
      className={cn(
        ...
        active 
          ...
          : "bg-primary/20 border-white/5 hover:border-white/10"
      )}
    >
      <Icon className={cn("h-4 w-4", color)} />
      <span className="text-[8px] font-bold uppercase text-secondary/60">{label}</span>
    </button>
  );
}
