...
# Hybrid VOSK + BROCKSTON speech recognition module
...
# Load BROCKSTON custom vocabulary
try:
    with open("vocab-alpha.json", "r") as f:
        custom_vocab = json.load(f)
except FileNotFoundError:
    print("Warning: vocab-alpha.json not found, using empty vocabulary")
    custom_vocab = {}

# Load VOSK model
MODEL_PATH = "vosk-model-small-en-us-0.15"
if not os.path.exists(MODEL_PATH):
    print("Warning: VOSK model not found - speech recognition disabled")
    model = None
else:
...
# Apply fixes, corrections, and BROCKSTON logic
