...
class SyncNetInstance(torch.nn.Module):
    ...
    def evaluate(self, opt, videofile):
        ...
        command = (
            "ffmpeg -loglevel error -y -i %s -async 1 -ac 1 -vn -acodec pcm_s16le -ar 16000 %s"
            % (videofile, os.path.join(opt.tmp_dir, opt.reference, "media/audio/audio.wav"))
        )
        output = subprocess.call(command, shell=True, stdout=None)
        ...
        sample_rate, audio = wavfile.read(
            os.path.join(opt.tmp_dir, opt.reference, "media/audio/audio.wav")
        )
