...
try:
    from research_module import BROCKSTONResearchModule
except ImportError:
    BROCKSTONResearchModule = None
    ...
    if data_path and os.path.exists(data_path):
        try:
            ...
            if not all(col in df.columns for col in required_columns):
                logger.error(
                    f"Data file {data_path} missing required columns: {required_columns}"
                )
