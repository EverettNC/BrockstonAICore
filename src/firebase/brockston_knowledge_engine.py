"""
BROCKSTON's Knowledge-First Reasoning Engine
...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("logs/knowledge_engine.log"), logging.StreamHandler()],
)
logger = logging.getLogger(__name__)


class BROCKSTON:
    def __init__(self, memory_dir: str = "./brockston_memory", brockston_instance=None):
        try:
            self.memory = MemoryMeshBridge(memory_dir=memory_dir)
            logger.info("BROCKSTON initialized with MemoryMeshBridge")
...
class KnowledgeEngine:
    ...
    def _store_knowledge(self, topic, knowledge):
        """Store knowledge in BROCKSTON's memory"""
        if self.brockston.memory is None:
            logger.error("Memory system not initialized")
            raise RuntimeError("Memory system not initialized")
        try:
            self.brockston.memory.store(topic, knowledge)
    ...
    def reason(
        ...
        # Step 2: Search BROCKSTON's learned knowledge
        knowledge_result = self._search_learned_knowledge(question)

        # Step 3: Check BROCKSTON's memory for relevant experiences
        ...
        combined_confidence = self._calculate_confidence(
            knowledge_result, memory_result
        )
    ...
    def _search_learned_knowledge(self, question: str) -> Dict[str, Any]:
        """
        Search BROCKSTON's learned knowledge base
                ...
                try:
                    with open(knowledge_file, "r") as f:
                        ...
                        if word_overlap > 0:
                            ...
                            if any(
                                phrase in content for phrase in question_lower.split()
                            ):
                                relevance *= 1.5
    ...
    def _search_memory(self, question: str) -> Optional[Dict[str, Any]]:
        """
        Search BROCKSTON's memory mesh for relevant experiences
    ...
    def _reason_from_knowledge(
        ...
        """
        Generate response using BROCKSTON's knowledge and local AI
        ...
        if self.local_reasoning and self.local_reasoning.ollama_available:
            system_prompt = f"""You are BROCKSTON, an AI with specialized knowledge.
...
Focus on being helpful, compassionate, and evidence-based."""
    ...
    def print_statistics(self):
        ...
        print("\n" + "=" * 60)
        print("📊 BROCKSTON'S KNOWLEDGE REASONING STATISTICS")
        print("=" * 60)
...
if __name__ == "__main__":
    print("Testing BROCKSTON's Knowledge Engine...\n")
