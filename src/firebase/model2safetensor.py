...
from modules_keypoint_detector import HEEstimator, KPDetector
from modules_mapping import MappingNet
from modules_generator import OcclusionAwareSPADEGenerator

from models_audio2pose import Audio2Pose
from audio2exp_models_networks import SimpleWrapperV2
from test_audio2coeff import load_cpk

size = 256
############ face vid2vid
config_path = os.path.join("src", "config", "config/facerender.yaml")
current_root_path = "."
...
audio2pose_yaml_path = os.path.join(
    current_root_path, "src", "config", "config/auido2pose.yaml"
)
...
audio2exp_yaml_path = os.path.join(
    current_root_path, "src", "config", "config/auido2exp.yaml"
)
...
load_cpk_facevid2vid_safetensor(
    "checkpoints/SadTalker_V0.0.2_" + str(size) + ".safetensors",
    kp_detector=kp_extractor,
    generator=generator,
    he_estimator=None,
)
