"""Self-Learning and Adaptation Engine for BROCKSTON.

This module implements autonomous learning capabilities for BROCKSTON,
...
memory_engine = MemoryEngine(file_path="brockston_memory/semantic_memory.json")
...
# --- Compatibility wrapper for BROCKSTON's brain ---
_engine_instance = None


def learn_from_text(text: str):
    """
    Directly embed crawled text and store in BROCKSTON's memory engine.
    """
    if not text or not text.strip():
        logger.warning("Received empty text for learning.")
        return "No text provided"

    # With OpenAI removed, if we want to retain embedding capability here, we need a 
    # stand-in embedding strategy (e.g. Voyage, local model, or similar). For now, we will 
    # just store the text without embeddings or skip this block entirely if no other embedder is available.
    try:
        # Just save the raw text if no embedding service is available natively
        memory_engine.save({"type": "web_ingest", "text": text})
